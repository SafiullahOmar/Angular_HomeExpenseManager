...
export class TableDatasourceService {
  ...
  getMonthList() {
    return this.http.get<any>(
      'https://localhost:44391/api/MonthsData/GetListOfMonths'
    );
  }
  ...
  getTableRows(monthYear: string, monthNumber: string, tableName: string) {
    ...
    return this.http.get<any>(
      'https://localhost:44391/api/MonthsData/GetTableData',
      {
        params: parameters,
      }
    );
  ...
  postTableRow(monthDataForBackEnd: any) {
    return this.http.post(
      'https://localhost:44391/api/MonthsData/InsertTableRow',
      monthDataForBackEnd,
      { responseType: 'text' }
    );
  }
  ...
  deleteTableRow(rowId: number) {
    return this.http.delete(
      'https://localhost:44391/api/MonthsData/DeleteTableRow/' + rowId,
      {
        responseType: 'text',
      }
    );
